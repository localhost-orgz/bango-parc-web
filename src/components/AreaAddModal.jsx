...
export default function AreaAddModal({ isOpen, onOpenChange, onSuccess }) {
  ...
  // Fetch facilities and reservation types for select options
  useEffect(() => {
    if (isOpen) {
      axiosInstance
        .get("/facility")
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      toast.success("Berhasil!", {
        description: `Area "${name}" berhasil ditambahkan.`,
      });
    ...
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[500px] rounded-none border border-[#0F131F]/15 bg-white p-0 gap-0">
        <DialogHeader className="p-6 border-b border-[#0F131F]/10">
          <DialogTitle className="font-crimson-pro text-2xl text-[#0F131F]">Tambah Area</DialogTitle>
          <DialogDescription className="text-sm text-black/40 mt-1">
            Masukkan rincian area baru di bawah ini.
          </DialogDescription>
        </DialogHeader>

        <form onSubmit={handleCreate} className="space-y-4 p-6 overflow-y-auto max-h-[60vh]">
          {/* Input Name */}
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="text-xs text-black/40">
              Pilih fasilitas yang tersedia untuk area ini.
            </div>
          </div>
